...
export default function BorderGlow({
  ...
  coneSpread?: number;
  /** Plays a single lap of glow around the edge when this turns true. */
  animated?: boolean;
  sweepDuration?: number;
  /** Peak edge proximity (0–100) during the sweep; lower is softer. */
  sweepPeak?: number;
  colors?: string[];
  fillOpacity?: number;
  border?: boolean;
  ...
  // One lap of light around the edge: fade in, circle once, fade out.
  useEffect(() => {
    ...
    // Short delay so the lap starts once the section has settled into view.
    animateValue({
      delay: 600,
      ...
      onUpdate: (v) => {
        const t = v / 100;
        const angle = angleStart + 360 * easeInOutCubic(t);
        const envelope = t < 0.2 ? t / 0.2 : t > 0.7 ? (1 - t) / 0.3 : 1;
        card.style.setProperty("--cursor-angle", `${angle.toFixed(2)}deg`);
        card.style.setProperty("--edge-proximity", String(sweepPeak * envelope));
      },
      onEnd: () => {
  ...
  }, [animated, sweepDuration, sweepPeak]);
